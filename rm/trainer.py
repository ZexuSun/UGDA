...
class RegressionTrainer(Trainer):
    ...
    def compute_loss(
        ...
        del model_inputs["rewards"]

        _, _, values = model(**model_inputs, output_hidden_states=True, return_dict=True)

        unwrapped_model: "PreTrainedModel" = self.accelerator.unwrap_model(self.model)
        if getattr(unwrapped_model.config, "model_type", None) == "chatglm":
            values = torch.transpose(values, 0, 1)
        
        #take the value of eos token as the reward
        pred_rewards = values[:, -1].reshape(-1, 1)
        ...
        loss += torch.nn.functional.mse_loss(rewards, pred_rewards).mean()

        loss = loss / batch_size
        if return_outputs:
            pred_rewards = torch.stack(pred_rewards)
            return loss, [loss, pred_rewards]
